...
def create_model():
    """
    Creates and returns a CNN model for MNIST classification
...
fig_accuracy = px.line(x=list(range(1, 6)), y=history.history['accuracy'], 
                      labels={'x': 'Epochs', 'y': 'Accuracy'}, 
                      title='Accuracy over epochs')
fig_accuracy.add_scatter(x=list(range(1, 6)), y=history.history['val_accuracy'], 
                        mode='lines', name='Valid accuracy')
fig_accuracy.show()

fig_loss = px.line(x=list(range(1, 6)), y=history.history['loss'], 
                   labels={'x': 'Epochs', 'y': 'Loss'}, 
                   title='Loss over epochs')
fig_loss.add_scatter(x=list(range(1, 6)), y=history.history['val_loss'], 
                     mode='lines', name='Valid loss')
fig_loss.show()
